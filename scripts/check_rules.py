...
def format_blocked_output(action: str, comment: str) -> str:
    ...
    # Footer
    if is_block:
        output.append(f"  {'This is a HARD rule and cannot be overridden.'}")
        output.append(f"{'=' * width}")
    else:
        output.append(f"  To override: include 'OVERRIDE: <reason>' in description")
        output.append(f"{'-' * width}")
...
def main():
    ...
    try:
        ...
        output = {
            "continue": False,
            "reason": format_blocked_output("block", f"[BLOCK #0] Rule check error: {e}")
        }
    ...
    if action == "block":
        # Use continue: false with stopReason
        output = {
            "continue": False,
            "stopReason": format_blocked_output("block", comment)
        }
        print(json.dumps(output))
        sys.exit(0)

    if action == "warn":
        # For warnings, also block but with different formatting
        output = {
            "continue": False,
            "stopReason": format_blocked_output("warn", comment)
        }
        print(json.dumps(output))
        sys.exit(0)
